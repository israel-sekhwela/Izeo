            ...
            <div id="requests-section">
                ...
                <div id="search-bar">
                    <form action="" method="GET">
                        <input type="text" style="float:left;" name="query" placeholder="Search projects..." />
                    </form>
                    <br /><br />
                </div>
                <div id="search-area">
                    ...
                    <?php
                        ...
                        {
                            $raw_results = mysqli_query($dbcon, "SELECT * FROM assignments WHERE yeargroup = '$yeargroup' AND (`subject` LIKE '%".$query."%') OR (`assignment` LIKE '%".$query."%') OR (`staff` LIKE '%".$query."%')");

                            if(mysqli_num_rows($raw_results) > 0)
                            {
                                $assignmentno = mysqli_num_rows($raw_results);
                                
                                echo "<h3>$assignmentno projects were found from your search</h3><br /><br />";

                                while($results = mysqli_fetch_array($raw_results))
                                {
                                    echo "<p><h3><b>" . $results['subject']."</b></h3>" . $results['assignment']."</p>";
                                    echo "<p><a href=../assignments/". $results["filename"] . ">" . $results["filename"] . "</a></p>";
                                    echo "<p><b>Due: " . $results["datedue"] . "</b></p>";
                                }

                            }
                            else
                            {
                                echo "<h3>No project(s) found</h3>";
                            }

                        }
                        else
                        {
                            echo "<h3>Please enter more than " . $min_length . " characters for improved project search</h3>";
                        }
                    ?>
                </div>
