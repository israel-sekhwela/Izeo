            ...
            <div id="requests-section">
                ...
                <div id="search-bar">
                    <form action="" method="GET">
                        <input type="text" style="float:left;" name="query" placeholder="Search forum discussions.." />
                    </form>
                    <br /><br />
                </div>
                <div id="search-area">
                    ...
                    <?php
                        ...
                        {
                            $raw_results = mysqli_query($dbcon, "SELECT * FROM tbl_forum WHERE (`subject` LIKE '%".$query."%') OR (`title` LIKE '%".$query."%')");

                            if(mysqli_num_rows($raw_results) > 0)
                            {
                                $forumfound = mysqli_num_rows($raw_results);
                                
                                echo "<h3>$forumfound forum discussions were found from your search</h3><br /><br />";

                                while($results = mysqli_fetch_array($raw_results))
                                {
                                    echo "<h3><a href='view-forum.php?forumid=". $results["forumid"] . "'> " . $results["subject"] . " - " . $results["title"] . "</a></h3>";
                                    echo "<b>By : <a href=profile.php?student=". $results["student"] . ">" . $results["student"] . "</a></b>";
                                }

                            }
                            else
                            {
                                echo "<h3>No forum discussions found</h3>";
                            }

                        }
                        else
                        {
                            echo "<h3>Please enter more than " . $min_length . " characters for improved forum search</h3>";
                        }
                    ?>
                </div>
